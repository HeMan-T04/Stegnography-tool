        ...
        if st.button("Hide", type="primary") and coverFile and dataFile and password:
            with st.spinner("runnning"):
                ...
                if coverFile.name.split(".")[-1] == "exe":
                    st.download_button("Download "+coverFile.name + " with hidden data", hiddenData,"hidden.exe", mime="application/x-msdownload")
                else:
                ...
                try:
                    ...
                    pass
                fileName = nextnonexistent(f'Files/{coverFile.name}')
